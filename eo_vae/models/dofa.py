...
class DOFAViT(nn.Module):
    ...
    def forward_features(self, x, wvs: torch.Tensor):
        # embed patches
...
def vit_base_patch14(**kwargs):
    ...
    # similar to RAE framwork, remove the layernorm normalization
...
class DOFAEncoderWrapper(nn.Module):
    ...
    def __init__(self, dofa_model, wavelengths, weights=None, out_indices=None) -> None:
        ...
        self.out_channels = [self.dofa_model.patch_embed.embed_dim] * len(
            self.out_indices
        )
...
def load_dofa_weights(
    ...
    if ckpt_data is not None:
        if ckpt_data.find('https://hf.co/') > -1:
            ...
            ckpt_data = huggingface_hub.hf_hub_download(
        ...
        if weights is not None:
            ...
            if input_size != 224:
                if (
                    'pos_embed' in checkpoint_model
                    and checkpoint_model['pos_embed'].shape
                    != state_dict['pos_embed'].shape
                ):
