from src.models.dynamic_conv import DynamicConv, get_1d_sincos_pos_embed_from_grid_torch, DynamicConv_decoder
...
class Dynamic_WG(DynamicConv):
    def __init__(self, wv_planes, inter_dim=128, kernel_size=3, stride=1, padding=1, embed_dim=128):
...
def main(epochs):
    ...
    weight_generator = Dynamic_decoder(wv_planes=128, inter_dim=128, kernel_size=3, stride=1, padding=1, embed_dim=128).cuda()

    optimizer = optim.AdamW(weight_generator.parameters(), lr=0.00001, weight_decay=0.0003)
    ...
    for i in range(epochs):
        ...
        train_one_epoch(i, weight_generator, t_model.decoder.conv_out, waves, optimizer, criterion, scaler)

    state_dict = {'weight_generator':weight_generator.weight_generator.state_dict(),\
